...
// Servlet 관련 코드를 작성하기 위해서는 HttpsServlet 클래스 상속 받아야한다.
// -> 상속 받았다고 해서 바로 Servlet 등록이 되는 것은 아님.
// 1. web.xml 작성 / 2. @WebServlet() 어노테이션 사용.
...
public class ExampleController2 extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 원래 Tomcat 9버전 때는
		// 전달 받은 값을 얻어와서 확인 -> 한글이 깨지는 문제 발생.
		// 해결 방법 : 전달 받은 데이터의 문자 인코딩을 서버에 맞게 변경.
		
//		req.setCharacterEncoding("UTF-8");
		// -> Tomcat 10버전 이상부터 자동으로 인코딩 처리를 해주기 때문에 작성하지 않아도 된다.
		...
		// 응답 화면 만들기.
		// -> Java에서 응답 화면을 작성하기 번거롭고 힘들다.
		// -> JSP가 대신 화면을 만듦 ( Servlet이 JSP에게 요청을 위임 )
		
		// JSP가 대신 응답 화면을 만들기 위해서
		...
		dispatcher.forward(req, resp); // RequestDispatcher 객체를 사용하여 현재 요청(req)과 응답(resp)을 지정한 JSP 페이지(result.jsp)로 전달하는 작업.
		// -> 현재 서블릿에서 처리하던 요청을 result.jsp로 전달하고
		//    제어권을 그 곳으로 넘김 ( == 위임함 )
